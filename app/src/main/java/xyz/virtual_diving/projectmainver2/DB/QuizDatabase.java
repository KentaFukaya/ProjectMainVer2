...
public class QuizDatabase {
    ...
    // Fromassets テスト
    public static ArrayList<QuizDetail> getQuizDetailsAllFromQuiztest(int fishId) {
        ArrayList<QuizDetail> quizDetails = new ArrayList<>();

//        QuizSQLiteOpenHelper helper = new QuizSQLiteOpenHelper(QuizActivity.getContext());
//        mDb = helper.getReadableDatabase();

        QuizSQLiteOpenFromAssets helper = new QuizSQLiteOpenFromAssets(QuizActivity.getContext(), "Quiztest.db", null,1);
        SQLiteDatabase db = helper.getReadableDatabase();

        Cursor c = db.query(QuizSQLiteOpenHelper.TABLE_NAME, FROM,"fishId = ?", new String[]{ "" + fishId }, null, null, ORDER_BY);//queryの実行
